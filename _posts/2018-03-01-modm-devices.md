...
don't expect any professional support. [Maybe you can hire me though](linked-in).
...
Configuring the USART1_TX signal on the popular STM32F103RBT.
</center>
...
So `USART1_TX` maps to pin PA6 with `USART1_REMAP1` or pin PA9 with `USART1_REMAP0`.
The STM32F1 series remap signals either in (overlapping) groups or not at all.
This is controlled by the `AFIO_MAPRx` registers, where we can find PA6/PA9 again:
...
only contains 60 device files. Ok, so ST recently added 37 devices of the new
STM32L4+ family, for which the DFG isn't adapted yet, but it's still ~19x less
files than devices.
...
`stats` also allows you to dump expanded JSON for prefix of devices and then
query that with the tool of your choice to, for example, get all the I<sup>2</sup>C
related alternate functions on port B for the STM32F4 device family.
Not sure why that's important, but it's possible.
...
own opinions and won't stop arguing until they've "won" (just ask about using
...
[@japaric](https://twitter.com/japaricious), who are currently tackeling some very hard issues, [like IO signal
grouping](http://blog.japaric.io/brave-new-io/#no-pin-overlap) or [DMA APIs](http://blog.japaric.io/safe-dma/).
...
_All images and all data is copyrighted by their respective owners and used here in fair use._